...
Render free tier is ~512MB. Holding a lock for an entire Speak stream would
stall chat; instead we only serialize short critical sections (model load /
single chunk synth / one graph run). GC runs after TTS sections only.
"""
